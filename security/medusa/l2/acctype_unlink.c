...
static int __init unlink_acctype_init(void)
{
...
medusa_do_unlink(const struct path *dir, struct dentry *dentry)
{
	...
	result = MED_DECIDE_RESULT(unlink_access, &access, &process, &file);
...
enum medusa_answer_t medusa_unlink(const struct path *dir, struct dentry *dentry)
{
	...
	if (!is_med_magic_valid(&(task_security(current)->med_object)) &&
	    ...
	    !MEDUSA_FALLBACK_REQUIRES_DECISION(unlink_access))
		return mad.ans;
