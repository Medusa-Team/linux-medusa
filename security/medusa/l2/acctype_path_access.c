 ...
 *
 * Note 1: To prepend also terminating NULL character of @str, @strlen
...
/**
 * medusa_get_path_from_string - prepend a @str in front of the end
 *	of newly allocated buffer
 * @str: string to place at the end of a buffer
 *
 * Returns newly allocated buffer with @str inserted at the end of it,
 * or NULL on allocation failure.
 *
 * Note: Buffers allocated with this function should be freed calling
...
/**
 * medusa_put_path - free a buffer used for pathname string
 * @pathbuf: buffer to free
 *
 * Free a buffer allocated with __getname() and filled by
 * d_absolute_path(), which fills a buffer starting from its
 * end. So to __putname() function it should be passed the right
 * pointer obtained from __getname().
 */
 ...
 *
 * Returns a pointer into the full pathname string or error code if an error occurs.
 * A buffer, which pointer to is returned, is allocated from the slab,
 * so callers have to free it by medusa_put_path().
 */
char *medusa_get_path(const struct path *path, const struct qstr *last, int lasttype)
{
	const char *lastname = NULL;
	char *lastptr, *buf = NULL, *pathbuf = NULL;
	int lastlen = 0, buflen = PATH_MAX;
	 ...
	 * want obtain always all paths in the same normalised form.
 ...
 * @path_to_redirect: in/out param; stores pointer to buffer, where is path (string),
 *	which is object of decision. If shouldn't be realised redirection,
 *	@path_to_redirect must be set to NULL using medusa_put_path() function call.
 *	This call will free buffer in correct way and set its argument to NULL.
 *	If @path_to_redirect on return is not NULL, underlaying path string is used
 *	for redirection to it.
 *
 * Returns decision answer, which can be one of:
 *	MED_FAKE_ALLOW - after return force terminating callers function with SUCCESS
 *	MED_DENY - after return deny executing callers function
 *	other values(s), i.e. MED_ALLOW - after return allow executing callers function
 *
 * After returning to caller:
 *	If return value is not (MED_DENY || MED_FAKE_ALLOW) and @path_to_redirect is
 *	not NULL, caller's path access is redirected to path pointed by @path_to_redirect
 *	output parameter. If return value is MED_DENY or MED_FAKE_ALLOW, value of output
 *	parameter @path_to_redirect *must be* NULL.
 */
medusa_answer_t medusa_path_access(const char *action, char **path_to_redirect)
{
	...
	if (!strncmp(*path_to_redirect, "/tmp/medusa_path", strlen("/tmp/medusa_path"))) {
		printk("MEDUSA PATH_ACCESS 1: act='%s', path_to_redirect='%s'", action, *path_to_redirect);

		if (!strcmp(*path_to_redirect, "/tmp/medusa_path_deny"))
			retval = MED_DENY;
		else if (!strcmp(*path_to_redirect, "/tmp/medusa_path_allow"))
			retval = MED_ALLOW;
		else if (!strcmp(*path_to_redirect, "/tmp/medusa_path_fake_allow"))
			retval = MED_FAKE_ALLOW;
		else if (!strcmp(*path_to_redirect, "/tmp/medusa_path_redirect")) {
			retval = MED_ALLOW;
		...
		medusa_put_path(path_to_redirect);
out:
		printk("MEDUSA PATH_ACCESS 2: act='%s', path_redirected_to='%s', retval=%d\n", \
