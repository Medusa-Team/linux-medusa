...
medusa_do_readlink(struct dentry *dentry)
{
	...
	result = MED_DECIDE_RESULT(readlink_access, &access, &process, &file);
...
enum medusa_answer_t medusa_readlink(struct dentry *dentry)
{
	...
	if (!is_med_magic_valid(&(task_security(current)->med_object)) &&
	    ...
	    !MEDUSA_FALLBACK_REQUIRES_DECISION(readlink_access))
		return mad.ans;
