...
medusa_do_truncate(const struct path *path)
{
	...
	result = MED_DECIDE_RESULT(truncate_access, &access, &process, &file);
...
enum medusa_answer_t medusa_truncate(const struct path *path)
{
	...
	if (!is_med_magic_valid(&(task_security(current)->med_object)) &&
	    ...
	    !MEDUSA_FALLBACK_REQUIRES_DECISION(truncate_access))
		return mad.ans;
	...
	if (MEDUSA_MONITORED_ACCESS_O(truncate_access, inode_security(path->dentry->d_inode))) {
		medusa_audit_apply_decision(&mad, medusa_do_truncate(path));
	}
audit:
