...
medusa_do_link(struct dentry *old_dentry, const char *newname)
{
	...
	result = MED_DECIDE_RESULT(link_access, &access, &process, &file);
				 ...
				 struct dentry *new_dentry)
{
	...
	if (err < 0) {
		medusa_audit_apply_local(&mad, MED_ERR,
