...
static ssize_t (*_to_user)(void *, size_t);
	...
	while (userlimit || teleport->cycle == tpc_FETCH) {
		...
		case tpc_FETCH:
			...
			case tp_PUTKCLASS:
				...
				memcpy(teleport->u.putkclass.cl.name, teleport->ip->args.putkclass.kclassdef->name, MEDUSA_COMM_KCLASSNAME_MAX < MEDUSA_KCLASSNAME_MAX ? MEDUSA_COMM_KCLASSNAME_MAX : MEDUSA_KCLASSNAME_MAX);
				teleport->data_to_user =
			...
			case tp_PUTEVTYPE:
				...
				memcpy(teleport->u.putevtype.ev.name, teleport->ip->args.putevtype.evtypedef->name, MEDUSA_COMM_EVNAME_MAX < MEDUSA_EVNAME_MAX ? MEDUSA_COMM_EVNAME_MAX : MEDUSA_EVNAME_MAX);
				...
				memcpy(teleport->u.putevtype.ev.ev_name[0], teleport->ip->args.putevtype.evtypedef->arg_name[0], MEDUSA_COMM_ATTRNAME_MAX < MEDUSA_ATTRNAME_MAX ? MEDUSA_COMM_ATTRNAME_MAX : MEDUSA_ATTRNAME_MAX);
				memcpy(teleport->u.putevtype.ev.ev_name[1], teleport->ip->args.putevtype.evtypedef->arg_name[1], MEDUSA_COMM_ATTRNAME_MAX < MEDUSA_ATTRNAME_MAX ? MEDUSA_COMM_ATTRNAME_MAX : MEDUSA_ATTRNAME_MAX);
				teleport->data_to_user =
			...
			if (teleport->cycle == tpc_HALT)
				break;
			/* fallthrough without gcc warning "statement may fall through */
			goto tpc_execute;
		case tpc_EXECUTE:
tpc_execute:
			switch (teleport->ip->opcode) {
			...
			case tp_PUTATTRS:
				...
				if (teleport->u.putattrs.current_attr >= 0 &&
				    teleport->ip->args.putattrs.attrlist[teleport->u.putattrs.current_attr].type == MED_END) {
					teleport->cycle = tpc_FETCH;
					break;
				}
